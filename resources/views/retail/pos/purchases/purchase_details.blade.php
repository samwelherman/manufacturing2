        ...
        <div class="row">
            ...
            <div class="col-12 col-md-12 col-lg-12">
               ...
               <div class="col-lg-10">
                 ...
                 <a class="btn btn-xs btn-primary"  onclick="return confirm('Are you sure?')"   href="{{ route('retail_purchase.edit', $purchases->id)}}"  title="" > Edit </a>          
            
                @endif

                   @if($purchases->status == 0 )        
              <a class="btn btn-xs btn-info"  title="Convert to Invoice" onclick="return confirm('Are you sure?')"    href="{{ route('retail_purchase.receive',$purchases->id)}}"  title="" >Approve Purchase </a>
                @endif

               @if($purchases->status == 1 || $purchases->status == 2)                      
                <a class="btn btn-xs btn-danger " data-placement="top"  href="{{ route('retail_purchase.pay',$purchases->id)}}"  title="Add Payment"> Pay Purchase  </a>    
           @endif  

      @if($purchases->status != 0 && $purchases->good_receive == 0)                        
              <a class="btn btn-xs btn-info" data-placement="top"  href="{{ route('retail_purchase.approve',$purchases->id)}}"    onclick="return confirm('Are you sure?')"  title="Good Receive"> Good Receive </a>   
           @endif  
             
             <a class="btn btn-xs btn-success"  href="{{ route('retail_purchase_pdfview',['download'=>'pdf','id'=>$purchases->id]) }}"  title="" > Download PDF </a>         
 ...
 <div class="col-lg-3 col-xs-3">
          ...
          <br>Status: 
                                   @if($purchases->status == 0)
                                            ...
                                            @elseif($purchases->status == 5)
                                            <span class="badge badge-danger badge-shadow">Received</span>
                                            @elseif($purchases->status == 6)
                                            <span class="badge badge-danger badge-shadow">Scanned and Paid</span>
                                            @endif
...
<!--
                                            ...
                                            <td class="">{{Carbon\Carbon::parse($row->expire_date)->format('M Y')}}  </td>
...
<td>{{number_format(($gland_total + $purchases->shipping_cost) - $purchases->discount ,2)}}  {{$purchases->exchange_code}}</td>
 ...
 @if(!@empty($purchases->due_amount < $purchases->purchase_amount + $purchases->purchase_tax + $purchases->shipping_cost  ))
     <tr>
<td colspan="4"></td>
                    <td>Paid Amount</p>
                    <td>{{number_format(( $purchases->purchase_amount + $purchases->purchase_tax + $purchases->shipping_cost ) - $purchases->due_amount,2)}}  {{$purchases->exchange_code}}</p>
                </tr>
...
<td colspan="4"></td>
<td>Shipping Cost </td>
<td>{{number_format($purchases->shipping_cost * $purchases->exchange_rate,2)}}  TZS</td>
</tr>

<tr>
<td colspan="4"></td>
<td>Total Amount</td>
<td>{{number_format($purchases->exchange_rate * (($gland_total + $purchases->shipping_cost)-$purchases->discount) ,2)}}   TZS</td>
</tr>

 @if(!@empty($purchases->due_amount <  $purchases->purchase_amount + $purchases->purchase_tax + $purchases->shipping_cost ))
     <tr>
                    <td>Paid Amount</p>
                    <td>{{number_format($purchases->exchange_rate * (( $purchases->purchase_amount + $purchases->purchase_tax + $purchases->shipping_cost ) - $purchases->due_amount),2)}}  TZS</p>
                </tr>

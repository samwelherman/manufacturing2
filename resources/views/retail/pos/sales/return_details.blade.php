...
<section class="section">
    <div class="section-body">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-12 col-lg-12">
               ...
               <div class="col-lg-10">
                 ...
                 <a class="btn btn-xs btn-primary"  onclick="return confirm('Are you sure?')"   href="{{ route('retail_credit_note.edit', $invoices->id)}}"  title="" > Edit </a>          
      ...
      @if($invoices->status == 0 && $invoices->status != 4 && $invoices->status != 3 && $invoices->good_receive == 0)                        
              <a class="btn btn-xs btn-info" data-placement="top"  href="{{ route('retail_credit_note.receive',$invoices->id)}}"  title="Good Receive">Approve Credit Note</a>   
           @endif  
             
             <a class="btn btn-xs btn-success"  href="{{ route('retail_credit_note_pdfview',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Download PDF </a>         
 ...
 <div class="col-lg-3 col-xs-3">
                                      ...
                                      <div class="col-lg-3 col-xs-3">
                                       ...
                                       <h5 class=mb0">REF NO : {{$invoices->reference_no}}</h5>
                                        <strong>Return from Invoice No:  {{$invoices->invoice->reference_no }}</strong><br>
                        ...
                        <th  style="color:white;">Total</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="">{{$i++}}</td>
                                            <?php
                                         $item_name = App\Models\Retail\InvoiceItems::find($row->return_item);
                                             $item= App\Models\Retail\Items::find($item_name->type);
                                        ?>
                                            <td class=""><strong class="block">{{$item->name}}</strong></td>
                                             <td class="">
                                           @if( $item_name ->category == 'Batch')
                                          {{ $item_name ->batch_number }}
                                          @else
                                           {{ $item_name ->serial_no }} 
                                                 @endif
                                                    </td>

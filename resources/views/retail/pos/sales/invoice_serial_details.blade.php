        ...
        <div class="row">
            ...
            <div class="col-12 col-md-12 col-lg-12">
               ...
               <div class="col-lg-10">
                 ...
                 <a class="btn btn-xs btn-primary"  onclick="return confirm('Are you sure?')"   href="{{ route('retail_serial_invoice.edit', $invoices->id)}}"  title="" > Edit </a>          
            
                @endif
                @if($invoices->invoice_status == 0)
                 <a class="btn btn-xs btn-primary"  onclick="return confirm('Are you sure?')"   href="{{ route('retail_serial_invoice.convert_to_invoice', $invoices->id)}}"  title="" > Convert To Invoice </a>          
            
                @endif


               @if($invoices->status != 0 && $invoices->status != 4 && $invoices->status != 3 && $invoices->good_receive == 1)                      
                <a class="btn btn-xs btn-danger " data-placement="top"  href="{{ route('retail_serial_pos_invoice.pay',$invoices->id)}}"  title="Add Payment"> Pay invoice  </a>    
           @endif  

      @if($invoices->status == 0 && $invoices->status != 4 && $invoices->status != 3 && $invoices->good_receive == 0)                        
              <a class="btn btn-xs btn-info" data-placement="top"  href="{{ route('retail_serial_invoice.receive',$invoices->id)}}"  title="Good Receive"> Approve Invoice</a>   
           @endif  
             
             <a class="btn btn-xs btn-success"  href="{{ route('retail_serial_pos_invoice_pdfview',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Download PDF </a>         
                  
  @if($invoices->good_receive == 1)                        
                  <a class="btn btn-xs btn-success"  href="{{ route('retail_serial_pos_note_pdfview',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Download Delivery Note</a>   
           @endif                         
    </div>
                        ...
                        <th  style="color:white;">Total</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="">{{$i++}}</td>
                                            <?php
                                         //$item_name = App\Models\Retail\Items::find($row->item_name);
                                         $item_name = App\Models\Retail\PurchaseSerialList::find($row->item_name);
                                        ?>

                        ...
                        @foreach($data as $key)
                            ...
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                 <td>{{$key->name}}</td>
                                 @php
                                      $pqty= App\Models\Retail\PurchaseHistory::where('item_id', $key->id)->sum('quantity');   
                                        $total_p+=$pqty;

                                       $sqty= App\Models\Retail\InvoiceHistory::where('item_id', $key->id)->where('type', 'Sales')->sum('quantity'); 
                                          $total_s+=$sqty;
                                       $cn= App\Models\Retail\InvoiceHistory::where('item_id', $key->id)->where('type', 'Credit Note')->sum('quantity');  
                                        $total_sr+=$cn;
                                        @endphp 
...
@foreach($data as $key)
 ...
 <tbody>
                    <?php                   
                        $account =App\Models\Retail\PurchaseHistory::where('item_id', $key->id)->orderBy('purchase_date','asc')->get();
                        ?>  
...
@foreach($data as $key)
 ...
 <tbody>
                    <?php                   
                        $account =App\Models\Retail\InvoiceHistory::where('item_id', $key->id)->orderBy('invoice_date','desc')->get();
                        ?>  
                 @foreach($account  as $a)
                                <tr>
                                              <td >{{$loop->iteration }}</td>
 ...
 <?php
                        ...
                        $q = App\Models\Retail\InvoiceHistory::where('item_id', $key->id)->where('type', 'Sales')->sum('quantity');
                        $r = App\Models\Retail\InvoiceHistory::where('item_id', $key->id)->where('type', 'Credit Note')->sum('quantity');
                        ?>  
<tfoot>
                    <tr>     

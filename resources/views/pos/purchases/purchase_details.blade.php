    ...
    <div class="section-body">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-12 col-lg-12">
              ...
              <a class="btn btn-xs btn-info"  title="Convert to Invoice" onclick="return confirm('Are you sure?')"    href="{{ route('purchase.approve',$purchases->id)}}"  title="" >Approve Purchase </a>
                @endif

               @if($purchases->status != 0 && $purchases->status != 4 && $purchases->status != 3 && $purchases->good_receive == 1)                      
                <a class="btn btn-xs btn-danger " data-placement="top"  href="{{ route('purchase.pay',$purchases->id)}}"  title="Add Payment"> Pay Purchase  </a>    
           @endif  

      @if($purchases->status != 0 && $purchases->status != 4 && $purchases->status != 3 && $purchases->good_receive == 0)                        
              <a class="btn btn-xs btn-info" data-placement="top"  href="{{ route('purchase.receive',$purchases->id)}}"  title="Good Receive"> Good Receive </a>   
           @endif  

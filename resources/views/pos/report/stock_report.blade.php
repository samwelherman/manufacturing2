...
?>
                     ...
                     @php
                        ...
                        $pqty= App\Models\POS\PurchaseHistory::where('item_id', $key->id)->where('type', 'Purchases')->whereBetween('purchase_date',[$start_date,$end_date])->sum(\DB::raw('quantity * price'));  
                         $dn= App\Models\POS\PurchaseHistory::where('item_id', $key->id)->where('type', 'Debit Note')->whereBetween('purchase_date',[$start_date,$end_date])->sum(\DB::raw('quantity * price'));                                  
                          $sqty= App\Models\POS\InvoiceHistory::where('item_id', $key->id)->where('type', 'Sales')->whereBetween('invoice_date',[$start_date,$end_date])->sum(\DB::raw('quantity * price'));
                          $cn= App\Models\POS\InvoiceHistory::where('item_id', $key->id)->where('type', 'Credit Note')->whereBetween('invoice_date',[$start_date,$end_date])->sum(\DB::raw('quantity * price'));
                                        @endphp 
  ...
  <div class="table-responsive">
                   ...
                   <td >@if(!empty($a->location)){{$a->store->name }}@endif</td>
                         ...
                         <?php
                              ...
                              $q = App\Models\POS\PurchaseHistory::where('item_id', $key->id)->where('type', 'Purchases')->whereBetween('purchase_date',[$start_date,$end_date])->sum(\DB::raw('quantity * price'));
                          $r = App\Models\POS\PurchaseHistory::where('item_id', $key->id)->where('type', 'Debit Note')->whereBetween('purchase_date',[$start_date,$end_date])->sum(\DB::raw('quantity * price'));
...
@foreach($data as $key)
  ...
  <div class="table-responsive">
                         ...
                         <?php
                                        ...
                                        $q = App\Models\POS\InvoiceHistory::where('item_id', $key->id)->where('type', 'Sales')->whereBetween('invoice_date',[$start_date,$end_date])->sum(\DB::raw('quantity * price'));
                                     $r = App\Models\POS\InvoiceHistory::where('item_id', $key->id)->where('type', 'Credit Note')->whereBetween('invoice_date',[$start_date,$end_date])->sum(\DB::raw('quantity * price'));
                                                ?>  
                        <tfoot>
                                            <tr>     
                                                <td ></td> <td ></td>
...
<script>
      ...
      $('.datatable-button-html5-basic').DataTable(
        ...
        buttons: [
          {extend: 'copyHtml5',title: 'STOCK VALUE FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}} ', footer: true},
           {extend: 'excelHtml5',title: 'STOCK VALUE FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true},
           {extend: 'csvHtml5',title: 'STOCK VALUE FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true},
            {extend: 'pdfHtml5',title: 'STOCK VALUE FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}', footer: true,customize: function(doc) {
doc.content[1].table.widths = [ '8%', '32%', '20%', '20%','20%'];
}
},
            {extend: 'print',title: 'STOCK VALUE FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true}

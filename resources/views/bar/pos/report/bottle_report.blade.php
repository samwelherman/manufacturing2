                        ...
                        <ul class="nav nav-tabs" id="myTab2" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link @if(empty($id)) active show @endif" id="home-tab2" data-toggle="tab"
                                    href="#home2" role="tab" aria-controls="home" aria-selected="true">Bottle Balance Report
                                    List</a>
                            </li>
...
?>
                        ...
                        @foreach($data as $key)
                            ...
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                 <td>{{$key->name}}</td>
                                 @php
                                      $pqty= App\Models\Bar\POS\InvoiceHistory::where('item_id', $key->id)->where('type', 'Sales')->sum('quantity');   
                                        $total_p+=$pqty;
                           ...
                           <td><a  href="#viewp{{$key->id}}"  data-toggle="modal" >{{number_format($sqty * $key->bottle,2)}}</a></td>
                                 <td><a  href="#views{{$key->id}}"  data-toggle="modal" >{{number_format($pqty,2)}}</a></td>     
                              <td>{{number_format(($sqty * $key->bottle)-($pqty),2)}}</td>                          
                            </tr>
                     ...
                     <td></td>
                       ...
                       <td>{{number_format($total_s- $total_p,2)}}</td>                               
                            </tr>
                        </tfoot>
                    </table>
...
<div class="table-responsive">
     <table class="table datatable-basic table-striped">
                 <thead>
                  ...
                  <tbody>
                      <?php                   
                          $account =App\Models\Bar\POS\InvoiceHistory::where('item_id', $key->id)->orderBy('invoice_date','desc')->get();
                          ?>  
   ...
   <?php
                          ...
                          $q = App\Models\Bar\POS\InvoiceHistory::where('item_id', $key->id)->where('type', 'Sales')->sum('quantity');
                           $r = App\Models\Bar\POS\InvoiceHistory::where('item_id', $key->id)->where('type', 'Credit Note')->sum('quantity');
                          ?>  
  <tfoot>
                      <tr>     
                          <td ></td> <td ></td>

...
?>
                     ...
                     <td><a class="nav-link" id="profile-tab2"  href="{{ route('store_purchase.show',$key->id)}}" role="tab"   aria-selected="false">{{$key->reference_no}}</a</td>
                      
                       <td>{{Carbon\Carbon::parse($key->due_date)->format('d/m/Y')}} </td>
                         <td>{{number_format($key->purchase_amount + $key->purchase_tax,2)}} {{$key->exchange_code}}</td>  
                                    <td>
                                   <?php      

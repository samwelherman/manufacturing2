...
?>
        ...
        <?php if (!empty($salary_info) && $salary_info->user_id == $v_employee->user_id) { ?>
                      ...
                      <div class = "input-group"> 
                @if (empty($salary_info)) 
                                <a href="#" class="btn btn-info btn-xs" title="View" data-toggle="modal" data-target="#appFormModal"  data-id="{{ $v_employee->id }}" data-type="template"   onclick="model({{ $v_employee->id }},'salary')">View Payment Details</a>
                      @else          

...
?>
                        ...
                        @foreach($data as $key)
                            ...
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                 <td>{{$key->name}}</td>
                                 @php
                                      $pqty= App\Models\Pharmacy\PurchaseHistory::where('item_id', $key->id)->sum('quantity');   
                                        $total_p+=$pqty;

                                       $sqty= App\Models\Pharmacy\InvoiceHistory::where('item_id', $key->id)->where('type', 'Sales')->sum('quantity'); 
                                          $total_s+=$sqty;
                                       $cn= App\Models\Pharmacy\InvoiceHistory::where('item_id', $key->id)->where('type', 'Credit Note')->sum('quantity');  
                                        $total_sr+=$cn;
                                        @endphp 
                           ...
                           <td><a  href="#viewp{{$key->id}}"  data-toggle="modal" >{{number_format($pqty,2)}}</a></td>
                              ...
                              <td>{{number_format($pqty-($sqty-$cn),2)}}</td>                          
                            </tr>
                     ...
                     <td></td>
                       ...
                       <td>{{number_format($total_p-($total_s - $total_sr),2)}}</td>                               
                            </tr>
                        </tfoot>
                    </table>
...
@foreach($data as $key)
 ...
 <tbody>
                    <?php                   
                        $account =App\Models\Pharmacy\PurchaseHistory::where('item_id', $key->id)->orderBy('purchase_date','asc')->get();
                        ?>  
 ...
 <?php
                        ...
                        $q = App\Models\Pharmacy\PurchaseHistory::where('item_id', $key->id)->sum('quantity');
...
@foreach($data as $key)
 ...
 <tbody>
                    <?php                   
                        $account =App\Models\Pharmacy\InvoiceHistory::where('item_id', $key->id)->orderBy('invoice_date','desc')->get();
                        ?>  
                 @foreach($account  as $a)
                                <tr>
                                              <td >{{$loop->iteration }}</td>
 ...
 <?php
                        ...
                        $q = App\Models\Pharmacy\InvoiceHistory::where('item_id', $key->id)->where('type', 'Sales')->sum('quantity');
                        $r = App\Models\Pharmacy\InvoiceHistory::where('item_id', $key->id)->where('type', 'Credit Note')->sum('quantity');
                        ?>  
<tfoot>
                    <tr>     

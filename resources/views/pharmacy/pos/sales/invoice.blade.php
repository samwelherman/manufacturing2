                   ...
                   @endif
                     ...
                     @endif
                                                ...
                                                <table class="table table-bordered" id="cart">
                                                  ...
                                                  $chk= App\Models\Pharmacy\PurchaseHistory::where('item_id',$i->type)->where('due_quantity','>','0')->where('location',$data->location)->where('added_by',auth()->user()->added_by)->get();

                                                 foreach($chk as $c){
                                                  if($c->expire_date != ''){
            $batch_list= App\Models\Pharmacy\PurchaseHistory::where('item_id',$i->type)->where('due_quantity','>','0')->where('location',$data->location)->where('added_by',auth()->user()->added_by)->where('expire_date', '>', $date)->get();
}

else{
  $batch_list= App\Models\Pharmacy\PurchaseHistory::where('item_id',$i->type)->where('due_quantity','>','0')->where('location',$data->location)->where('added_by',auth()->user()->added_by)->get();

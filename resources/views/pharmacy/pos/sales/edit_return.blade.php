                        ...
                        <ul class="nav nav-tabs" id="myTab2" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link @if(empty($id)) active show @endif" id="home-tab2" data-toggle="tab"
                                    href="#home2" role="tab" aria-controls="home" aria-selected="true">Credit Note
                                     List</a>
                            </li>
                            <li class="nav-item">
                                            ...
                                            <tr>
                                                ...
                                                <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0"
                                                  ...
                                                  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0"
                                                    rowspan="1" colspan="1"
                                                    aria-label="Platform(s): activate to sort column ascending"
                                                    style="width: 156.484px;">Invoice No</th>
                                                <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0"
                                                    rowspan="1" colspan="1"
                                                    aria-label="Platform(s): activate to sort column ascending"
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="gradeA even" role="row">
                                                ...
                                                <td>
                                                    <a class="nav-link" id="profile-tab2"
                                                            href="{{ route('pharmacy_credit_note.show',$row->id)}}" role="tab"
                                            ...
                                            <div class="form-inline">
                                                    ...
                                                    <a class="list-icons-item text-primary"
                                                        ...
                                                        href="{{ route('pharmacy_credit_note.edit', $row->id)}}"><i
                                                            class="icon-pencil7"></i></a>&nbsp
                                                            @endif

                                                    {!! Form::open(['route' => ['pharmacy_credit_note.destroy',$row->id],
                                                    'method' => 'delete']) !!}
                                 ...
                                 &nbsp
                                       ...
                                       <div class="dropdown-menu">
                                                            ...
                                                            <li> <a class="nav-link" id="profile-tab2"
                                                                    href="{{ route('pharmacy_credit_note.receive',$row->id)}}"
                                                                    role="tab"
                                                                    aria-selected="false"  onclick="return confirm('Are you sure?')">Approve Credit Note</a>
                                                            </li>
                                                            @endif
                                                             @if($row->status != 0 && $row->status != 4 && $row->status != 3 && $row->good_receive == 1)
                                                            <li> <a class="nav-link" id="profile-tab2"
                                                                    href="{{ route('pharmacy_credit_note.pay',$row->id)}}"
                                                                    role="tab"
                                                            ...
                                                            <li class="nav-item"><a class="nav-link" title="Cancel"
                                                                    ...
                                                                    href="{{ route('pharmacy_credit_note.cancel', $row->id)}}">Cancel
                                                                  Credit Note</a></li>
                                        @endif
                                   <li class="nav-item"><a class="nav-link" title="PDF"
                                                                    href="{{ route('pharmacy_credit_note_pdfview',['download'=>'pdf','id'=>$row->id]) }}">Download PDF</a></li>
                                       </div>
                                       </div>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-sm-12 ">
                                                @if(isset($id))
                                                {{ Form::model($id, array('route' => array('pharmacy_credit_note.update', $id), 'method' => 'PUT')) }}
                                              
                                                @else
                                                {{ Form::open(['route' => 'pharmacy_credit_note.store']) }}
                                                @method('POST')
                                                @endif
                                                ...
                                                <div class="form-group row">
                                                   ...
                                                   <label class="col-lg-2 col-form-label">Invoice </label>
                                                    <div class="col-lg-4">
                                                            ...
                                                            <select class="form-control m-b invoice" name="invoice_id" required
                                                                id="invoice_id">
                                                                ...
                                                                <option @if(isset($data))
                                                                    {{  $data->invoice_id == $row->id  ? 'selected' : ''}}
                                                ...
                                                <table class="table table-bordered" id="cart">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        @foreach ($items as $i)
                                                         ...
                                                         <?php
                                                              $inv= App\Models\Pharmacy\InvoiceItems::find($i->return_item);
                                                              if($inv->category == 'Batch'){
                                                               $date = today()->format('Y-m-d');                                                               
           $name=App\Models\Pharmacy\PurchaseHistory::where('item_id',$inv->type)->where('expire_date', '>', $date)->where('added_by',auth()->user()->added_by)->orderBy('expire_date', 'ASC')->get();
                                                             
                                                               }

                                                      else if($inv->category == 'Serial'){
                                                            $name=App\Models\Pharmacy\PurchaseSerialList::where('added_by',auth()->user()->added_by)->get();  
                                                              $due=1; 
                                                            }
                                                              ?>
                                                        <tr class="line_items">
...
console.log(id);
        $.ajax({
            url: '{{url("pos/pharmacy_sales/pharmacy_findinvoice")}}',
            type: "GET",
            data: {
                id: id
            },
...
console.log(id);
        $.ajax({
            url: '{{url("pos/pharmacy_sales/pharmacy_editshowInvoice")}}',
            type: "GET",
            data: {
                id: id,
            },
            ...
            success: function(response) {
                ...
                $.each(response,function(key, value)
                {
                     ...
                     $('#cart > tfoot').append(response.html);
...
console.log(id);
        $.ajax({
            url: '{{url("pos/pharmacy_sales/pharmacy_findinvQty")}}',
            type: "GET",
            data: {
                id: id,
              item: item,

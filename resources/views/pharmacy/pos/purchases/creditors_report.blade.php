...
<br>
@php
$center=App\Models\Pharmacy\Supplier::where('id',$account_id)->first();
@endphp
...
?>
                     ...
                     <td>
                      @if($key->type == 'Batch')
                        <a class="nav-link" id="profile-tab2"  href="{{ route('pharmacy_purchase.show',$key->id)}}" role="tab"   aria-selected="false">{{$key->reference_no}}</a>
                           @else
                                <a class="nav-link" id="profile-tab2"  href="{{ route('pharmacy_purchase_serial.show',$key->id)}}" role="tab"   aria-selected="false">{{$key->reference_no}}</a>
                                                    @endif
                            </td>

...
<td>
<select name="tariff_type[]" class="form-control m-b type" id="type" required>
...
?>
<td >
<div class="auto" id="auto" style="display:{{ $items->tariff_type != 'Manual' ? 'block' : 'none'}};">
<select name="{{$items->tariff_type != 'Manual' ? 'item_name[]' : ''}}" class="form-control m-b item_name" id="name"><option value="">Select Tariff</option>
@if(!empty($tariff))
...
<div class="auto1" id="auto1" style="display:{{ $items->tariff_type == 'Manual' ? 'block' : 'none'}};"><textarea name="{{$items->tariff_type == 'Manual' ? 'item_name[]' : ''}}" class="form-control item_name" id="name1" >{{ isset($items) ? $items->item_name : ''}}</textarea></div>
</td>
            ...
            success: function(response) {
               ...
               $("#name1").empty();
                ...
                $.each(response,function(key, value)
                {
                    ...
                    $("#name").append('<option value=' + value.id+ '>' + value.zone_name + ' -  ' + value.weight + '</option>');

...
<section class="section">
    <div class="section-body">
      ...
      @if($invoices->status == 0 && $invoices->status != 4 && $invoices->status != 3 && $invoices->good_receive == 0)                        
              <a class="btn btn-xs btn-info" data-placement="top"  href="{{ route('orders.receive',$invoices->id)}}"  title="Good Receive">Deliver Order </a>   
           @endif  
...
?>
                                      ...
                                      <td class="">{{ $row->payment->account_name }}</td>
                                         <td> <a class="nav-link"  href="{{ route('orders_payment_pdfview',['download'=>'pdf','id'=>$row->id]) }}"  title="" > Download PDF </a>       </td>   
                                        </tr>
                                        @endforeach

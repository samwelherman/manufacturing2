...
<br>
@php
$center=App\Models\Bar\POS\Client::where('id',$account_id)->first();
@endphp
...
?>
                        ...
                        $dueDate = strtotime($key->due_date);
                         ...
                         $invoice_due =($key->invoice_amount + $key->invoice_tax) -$key->due_amount;
                        
                        ?>
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                      <td><a class="nav-link" id="profile-tab2"  href="{{ route('bar_invoice.show',$key->id)}}" role="tab"   aria-selected="false">{{$key->reference_no}}</a</td>                         
                       <td>{{Carbon\Carbon::parse($key->due_date)->format('d/m/Y')}} </td>
                         <td>{{number_format($key->invoice_amount + $key->invoice_tax,2)}} {{$key->exchange_code}}</td>     
                                    <td>
                                   <?php      
                       ...
                       $total5 = $total5 + $invoice_due;                         
                         ...
                         $total6 = $total6 + $invoice_due;                    
                            ...
                            <td>{{number_format($key->due_amount,2)}} {{$key->exchange_code}}</td>                      

                                        ...
                                        <tbody>
                                            ...
                                            <div class="form-inline">
                                               ...
                                               <a class="list-icons-item text-success"
                                                    href="{{ route("restaurant_pos_issue.approve", $row->id)}}" onclick="return confirm('Are you sure, You want to Approve')" title="Change Status">
                                                    <i class="icon-checkmark3"></i>
                                                </a>&nbsp&nbsp                                             

                                                    <a class="list-icons-item text-primary"
                                                        href="{{ route("restaurant_pos_issue.edit", $row->id)}}">
                                                        <i class="icon-pencil7"></i>
                                                    </a>&nbsp
                                                   
                                                    {!! Form::open(['route' => ['restaurant_pos_issue.destroy',$row->id],
                                                    'method' => 'delete']) !!}
                                                  {{ Form::button('<i class="icon-trash"></i>', ['type' => 'submit', 'style' => 'border:none;background: none;', 'class' => 'list-icons-item text-danger', 'title' => 'Delete', 'onclick' => "return confirm('Are you sure?')"]) }}
                                                    ...
                                                    @else
                                                    <a class="nav-link"
                                                        href="{{ route("restaurant_pos_issue.edit", $row->id)}}"  data-toggle="modal" href=""  value="{{ $row->id}}" data-type="issue" data-target="#appFormModal"
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-sm-12 ">
                                                     @if(isset($id))
                                                {{ Form::model($id, array('route' => array('restaurant_pos_issue.update', $id), 'method' => 'PUT')) }}
                                                @else
                                                {{ Form::open(['route' => 'restaurant_pos_issue.store']) }}
                                                @method('POST')
                                                @endif
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-lg-4">
                                                        <select class="form-control m-b start" name="start" required
                                                                id="start">
...
<script>
    ...
    console.log(item);
            $.ajax({
                url: '{{url("restaurant/purchases/findQuantity")}}',
                type: "GET",
                data: {
                    id: id,
                  item: item,
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('change', '.type', function() {
            var id = $(this).val();
            $.ajax({
                url: '{{url("restaurant/purchases/findIssueService")}}',
                type: "GET",
                data: {
                    id: id
                },
...
$.ajax({
    type: 'GET',
     url: '{{url("restaurant/purchases/purchaseModal")}}',
    data: {
        'id': id,
        'type':type,
    },

...
<section class="section">
    <div class="section-body">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-12 col-lg-12">
               ...
               <div class="col-lg-10">
                 ...
                 <a class="btn btn-xs btn-primary"  onclick="return confirm('Are you sure?')"   href="{{ route('restaurant_debit_note.edit', $invoices->id)}}"  title="" > Edit </a>          
      ...
      @if($invoices->status == 0 && $invoices->status != 4 && $invoices->status != 3 && $invoices->good_receive == 0)                        
              <a class="btn btn-xs btn-info" data-placement="top"  href="{{ route('restaurant_debit_note.receive',$invoices->id)}}"  title="Good Receive"> Approve Debit Note </a>   
           @endif  
             
             <a class="btn btn-xs btn-success"  href="{{ route('restaurant_debit_note_pdfview',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Download PDF </a>         
 ...
 <div class="col-lg-3 col-xs-3">
                                      ...
                                      <div class="col-lg-3 col-xs-3">
                                       ...
                                       <h5 class="mb0">REF NO : {{$invoices->reference_no}}</h5>
                                        <strong>Return from Purchases No:  {{$invoices->purchase->reference_no }}</strong><br>
                                      Return Date : {{Carbon\Carbon::parse($invoices->return_date)->format('d/m/Y')}}                  
              <br>Due Date : {{Carbon\Carbon::parse($invoices->due_date)->format('d/m/Y')}}                                          
           <br>Invoices Agent: {{$invoices->supplier->name }} 
                  ...
                  <br> Email : <a href="mailto:{{$settings->email}}">{{$settings->email}}</a>                                                               
                   <br>TIN : {{$settings->tin}}
                                    ...
                                    <div class="col-lg-6 col-xs-6">
                                       ...
                                       <h4 class="mb0"> {{$invoices->supplier->name}}</h4>
                                      {{$invoices->supplier->address}}   
                                     <br>Phone : {{$invoices->supplier->phone}}                  
                                    <br> Email : <a href="mailto:{{$invoices->supplier->email}}">{{$invoices->supplier->email}}</a>                                                               
                                    <br>TIN : {{!empty($invoices->supplier->TIN)? $invoices->supplier->TIN : ''}}
...
<td>{{number_format($gland_total - $invoices->discount ,2)}}  {{$invoices->exchange_code}}</td>
 ...
 @if(!@empty($invoices->due_amount < $invoices->purchase_amount + $invoices->purchase_tax))
     <tr>
<td colspan="4"></td>
                    <td>Paid Amount</p>
                    <td>{{number_format(($invoices->purchase_amount + $invoices->purchase_tax) - $invoices->due_amount,2)}}  {{$invoices->exchange_code}}</p>
                </tr>
...
<td>{{number_format($invoices->exchange_rate * ($gland_total-$invoices->discount) ,2)}}   TZS</td>
 ...
 @if(!@empty($invoices->due_amount < $invoices->purchase_amount + $invoices->purchase_tax))
     <tr>
                    <td>Paid Amount</p>
                    <td>{{number_format($invoices->exchange_rate * (($invoices->purchase_amount + $invoices->purchase_tax) - $invoices->due_amount),2)}}  TZS</p>
                </tr>
...
?>
                                            ...
                                            href="{{ route('debit_note_payment.edit', $row->id)}}"><i
